...
class SpecTypeController extends Controller
{
    ...
    {
        $spectypes = SpecType::latest()->paginate(15);
    ...
    {
        $spec_type = SpecType::where('name', $request->name)->firstOrFail();
    ...
    {
        if ($request->term) {
            $spectypes = SpecType::where('name', 'like', '%' . $request->term . '%')->pluck('name')->toArray();
